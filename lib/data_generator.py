...
SELECT if({new_name!r} IN Names, 1, 0) AS Result"""
    ...
    @staticmethod
    def generate_random_uint_value(min_value: int = 0, max_value: int = 1000) -> int:
...
class ShopDataGenerator(DataGeneratorClickhouse):
    PROVIDER_NAMES = ["China", "Russia", "USA", "Canada", "EU", "Africa", "Thailand", "Australia"]
    STORAGE_NAMES = ["Warehouse", "Shop", "Secret"]
    ...
    def __init__(
        ...
        provider_count: int = 500,
        storage_count: int = 500,
        delivery_count: int = 1000,
        product_count: int = 1000,
        delivery_products_line_count: int = 700,
        invoice_product_line_count: int = 300,
    ...
    def generate_delivery_product_line_row(self, to_insert: bool = True) -> DeliveryProductsLine:
        ...
        quantity = self.generate_random_uint_value(min_value=1, max_value=100)
...
GROUP BY StorageID;"""
        ...
        if invoice_quantity.empty:
            ...
            quantity = self.generate_random_uint_value(min_value=1, max_value=int(max_value["QuantitySum"]))
        else:
            ...
            if int(max_value["QuantityDiff"]) < 0:
                return None
            quantity = self.generate_random_uint_value(min_value=1, max_value=int(max_value["QuantityDiff"]))
